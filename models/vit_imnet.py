...
class ViT_trans(nn.Module):
    def __init__(self, *, img_size, img_size_trans, patch_size, patch_size_trans, num_classes, dim, depth, heads, mlp_dim_ratio, channels = 3, num_classes_trans,
        ...
        self.num_patches_trans = (img_size_trans // patch_size_trans) * (img_size_trans // patch_size_trans)
        ...
        if not is_SPT:
            ...
            self.to_patch_embedding_trans = nn.Sequential(
                Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = patch_size_trans, p2 = patch_size_trans),
                nn.Linear(channels * patch_size_trans * patch_size_trans, self.dim)
            )
            
        else:
            self.to_patch_embedding = ShiftedPatchTokenization(self.num_patches, 3, self.dim, patch_size, is_pe=True, is_Coord=is_Coord)
            self.to_patch_embedding_trans = ShiftedPatchTokenization(self.num_patches_trans, 3, self.dim, patch_size_trans, is_pe=True, is_Coord=is_Coord)
        ...
        self.mlp_head_trans = nn.Sequential(
            nn.LayerNorm(self.dim),
            nn.Linear(self.dim, num_classes_trans)
        )
    ...
    def forward(self, img):
        ...
        x = self.to_patch_embedding_trans(img)
        ...
        if not self.is_Coord:
            x += self.pos_embedding_trans[:, :(n + 1)]
        x = self.dropout(x)

        x = self.transformer(x)      
        
        return self.mlp_head_trans(x[:, 0])
