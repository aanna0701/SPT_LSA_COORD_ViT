...
import models_.layers as layers
import models_.resnet_dnn_block as resnet_dnn
import models_.resnet_mcdo_block as resnet_mcdo
import models_.preresnet_dnn_block as preresnet_dnn
import models_.preresnet_mcdo_block as preresnet_mcdo
import models_.classifier_block as classifier

from functools import partial
from itertools import cycle
from einops import rearrange
from models_.layers import conv1x1, DropPath
from models_.attentions import Attention2d
...
class LocalAttention(nn.Module):
    ...
    def __init__(self, dim_in, dim_out=None, *,
                 window_size=7, k=1, is_LSA=False, is_Coord=False,
        ...
        self.mask = None
        if is_LSA:
            self.mask = torch.eye((window_size**2), (window_size**2))
    ...
    def forward(self, x, mask=None):
        ...
        x, attn = self.attn(x, mask, self.mask)
...
class StemB(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, is_SPT=False, pool=True):
        super().__init__()

        self.layer0 = []
        if is_SPT:
            self.layer0.append(PatchShifting(2))
            dim_in *= 5
        if pool:
...
class AlterNet(nn.Module):
    ...
    def __init__(self, block1, block2, *,
        ...
        self.layer1 = self._make_layer(block1, block2, 64, 64,  
                                       ...
                                       , is_LSA=is_LSA, is_SPT=is_SPT, is_Coord=is_Coord
                                       , **block_kwargs)
    ...
    def _make_layer(block1, block2, in_channels, out_channels, num_block1, num_block2, stride, heads, sds
        ...
        for alt, stride, sd in zip(alt_seq, stride_seq, sds):
            if not alt:
                block = block1
                seq.append(block(channels, out_channels, stride=stride, sd=sd, heads=heads, is_SPT=is_SPT, **block_kwargs))
            else:
                block = block2
                seq.append(block(channels, out_channels, stride=stride, sd=sd, heads=heads, is_LSA=is_LSA, is_Coord=is_Coord, **block_kwargs))
...
def dnn_50(img_size=32, num_classes=1000, stem=False, name="alternet_50", is_LSA=False, is_SPT=False, is_Coord=False,
           **block_kwargs):
    return AlterNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=True if img_size > 32 else False),
                    ...
                    num_classes=num_classes, name=name, is_LSA=is_LSA, is_SPT=is_SPT, is_Coord=is_Coord
                    , **block_kwargs)


def dnn_101(img_size=32,num_classes=1000, stem=True, name="alternet_101", is_LSA=False, is_SPT=False, is_Coord=False,
            **block_kwargs):
    return AlterNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=True if img_size > 32 else False),
...
def dnn_152(img_size=32, num_classes=1000, stem=True, name="alternet_152", is_LSA=False, is_SPT=False, is_Coord=False
            , **block_kwargs):
    return AlterNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=True if img_size > 32 else False),
