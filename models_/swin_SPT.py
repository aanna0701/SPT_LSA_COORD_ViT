    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, 
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, is_SCL=False, is_last=True if (i == depth-1 and is_last) else False)
            for i in range(depth)])

        # patch merging layer
        if downsample:
            
            self.downsample = ShiftedPatchTokenization(input_resolution[0]*input_resolution[1], dim, dim*2, 2, is_Coord=False)
...
class SwinTransformer_SPT(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = ShiftedPatchTokenization(img_size**2, 3, embed_dim, patch_size, is_pe=True, is_Coord=False)
