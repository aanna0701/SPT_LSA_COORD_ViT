...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = CoordLinear(dim, dim * 3, bias=qkv_bias, exist_cls_token=False)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = CoordLinear(dim, dim, exist_cls_token=False)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, is_last=False):
        super().__init__()
        self.dim = dim
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, is_SCL=True)
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, 
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, is_last=True if (i == depth-1 and is_last) else False)
...
class SwinTransformer_Coord(nn.Module):
    r""" Swin Transformer
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            # print(x.shape)
            b, n, _ = x.shape
            coords = self.addcoords(b, int(math.sqrt(n)), int(math.sqrt(n)))
            x = layer(x, coords, coords_attnd)
